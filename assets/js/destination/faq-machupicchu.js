/**
 * FAQ Machu Picchu - Versión PROFESIONAL
 * Cierra otras preguntas al abrir una nueva y al cambiar categoría
 */
console.log('🚀 FAQ Machu Picchu: Iniciando versión profesional...');

(function() {
    console.log('🔄 FAQ Machu Picchu: Cargando...');
    
    // Esperar a que el DOM esté listo
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', initFAQMachuPicchu);
    } else {
        initFAQMachuPicchu();
    }
    
    function initFAQMachuPicchu() {
        const faqSection = document.getElementById('machupicchu-faq');
        
        if (!faqSection) {
            console.error('❌ ERROR: No se encontró #machupicchu-faq');
            return;
        }
        
        console.log('✅ FAQ Machu Picchu encontrado');
        ...
        console.log('🎉 FAQ Machu Picchu configurado profesionalmente!');
    }
    ...
    function initProfessionalAccordion() {
        ...
        const faqItems = document.querySelectorAll('#machupicchu-faq .faq-item');
    ...
    function closeAllFAQItemsExcept(exceptItem = null) {
        const allItems = document.querySelectorAll('#machupicchu-faq .faq-item.active');
    ...
    function closeAllFAQItems() {
        const allItems = document.querySelectorAll('#machupicchu-faq .faq-item');
        allItems.forEach(item => {
            closeFAQItemProfessional(item);
        });
    }
    ...
    function initProfessionalFilter() {
        ...
        const buttons = document.querySelectorAll('#machupicchu-faq .category-btn');
    ...
    function showAllCategoriesProfessional() {
        const groups = document.querySelectorAll('#machupicchu-faq .faq-group');
    ...
    function showOnlyCategoryProfessional(category) {
        const allGroups = document.querySelectorAll('#machupicchu-faq .faq-group');
    ...
    window.faqMachuPicchuPro = {
        // Abrir una pregunta específica (cierra las demás)
        openQuestion: function(questionText) {
            const items = document.querySelectorAll('#machupicchu-faq .faq-item');
            let found = false;
        ...
        filterByCategory: function(category) {
            const button = document.querySelector(`#machupicchu-faq .category-btn[data-category="${category}"]`);
            if (button) {
                button.click();
                return true;
            }
        ...
        getActiveCategory: function() {
            const activeBtn = document.querySelector('#machupicchu-faq .category-btn.active');
            return activeBtn ? activeBtn.getAttribute('data-category') : 'all';
        },
        
        // Obtener pregunta activa
        getActiveQuestion: function() {
            const activeItem = document.querySelector('#machupicchu-faq .faq-item.active');
            if (activeItem) {
    ...
    console.log('🔧 Funciones profesionales disponibles: window.faqMachuPicchuPro');
